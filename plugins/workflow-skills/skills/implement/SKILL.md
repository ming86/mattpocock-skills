...
4. Derive validation and other supporting work from the behavior, outcome, or uncertainty this work needs to check. Use the smallest useful check or representative workflow that could expose a relevant failure, together with checks the project specifically requires. Keep supporting process proportional to the task and its consequences. A spec section, ticket boundary, or available check does not by itself expand that scope. When a broader interpretation would add substantial work, make the relevant boundary explicit.
