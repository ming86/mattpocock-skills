...
When the contribution can be checked cheaply and directly, prefer a targeted ablation: remove or simplify one candidate, compare the relevant behavior, property, or metric with and without it, and use the result as evidence. Otherwise, reason through the same counterfactual without manufacturing an experiment.
