...
Choose the test surface from the spec, ticket, repository architecture, existing tests, and call sites. If different choices would materially change architecture, scope, or what the test proves, treat the test surface as a design decision rather than choosing it solely for testing convenience.
