...
6. **Validate the result.** Check the reconciled behavior at the narrowest useful level that could reveal an incorrect merge, together with any checks the project specifically requires. Pay particular attention to behavior that either side was intentionally changing or preserving; broaden validation only when the conflict or its consequences justify it.
