...
5. **Report important incompatibility.** If the evidence does not establish which intent should win, surface the conflict rather than silently choosing. Do the same when the choice would materially change behavior, scope, compatibility, or architecture.
...
7. **Separate resolution mechanics from operation-level decisions.** Resolve the conflicted files and mark or stage resolved paths when that is part of the current conflict-resolution work. Continuing a rebase, creating a commit, aborting the operation, or changing merge strategy are separate Git-workflow decisions; this skill does not imply them.
