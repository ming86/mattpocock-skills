...
Implicit analytical skills such as `research`, `grilling`, `diagnosing-bugs`, `domain-modeling`, and `codebase-design` route normally. `tdd` routes when test-first development is explicitly requested or established by project instructions for the current work.
