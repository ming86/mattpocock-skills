...
If no durable domain artifact exists, do not create one automatically for incidental terminology. When a working model has cross-context value, first reuse the effort's existing durable location or follow the project's configured domain or durable-state convention. A local working-state area is appropriate when the model is provisional or too detailed for shared project truth. If no convention exists, use a simple work-centered artifact only when preserving the model is justified, and follow governing consultation rules when deciding whether a new shared or canonical domain artifact should be created.
