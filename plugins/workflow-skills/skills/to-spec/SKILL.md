...
If a material decision is still unresolved and the spec would have to invent an answer, mark that point explicitly unresolved instead of silently choosing.
...
5. Store the spec where the project already keeps the current work state, or follow the project's tracker, documentation, or durable-state convention. Reuse an existing work location instead of creating a parallel one. A configured local working-state area is appropriate for a personal or provisional spec; an existing shared spec, issue, or design document may already be the source of truth. If no convention exists, keep the spec in the simplest work-centered location that can serve as a stable reference for the work. Do not require a tracker when another durable document fits the work better.
